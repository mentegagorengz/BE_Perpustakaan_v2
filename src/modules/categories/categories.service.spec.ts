...
describe('CategoriesService', () => {
  ...
  describe('create', () => {
    it('should create and persist a category, returning the saved entity', async () => {
      const dto = { name: 'Fiction' };
      const entity = { id: 1, ...dto } as Category;
  ...
  describe('findAll', () => {
    it('should return paginated result with correct meta (snake_case, has_next_page/has_prev_page)', async () => {
      const data = [{ id: 1, name: 'A' }] as Category[];
    ...
    it('should filter by name using Like when search is provided', async () => {
      ...
      expect(repo.findAndCount).toHaveBeenCalledWith(
        expect.objectContaining({ where: { name: Like('%fic%') } }),
      );
    });
  });
  ...
  describe('findOne', () => {
    it('should return the category when found', async () => {
      const entity = { id: 1, name: 'A' } as Category;
  ...
  describe('update', () => {
    it('should merge changes and save when the category exists', async () => {
      const existing = { id: 1, name: 'Old' } as Category;
  ...
  describe('remove', () => {
    it('should remove the category when it exists', async () => {
      const existing = { id: 1, name: 'A' } as Category;
      repo.findOne.mockResolvedValue(existing);
