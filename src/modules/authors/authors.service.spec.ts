...
describe('AuthorsService', () => {
  ...
  describe('create', () => {
    it('should create and persist an author, returning the saved entity', async () => {
      const dto = { name: 'Robert C. Martin', bio: 'Author' };
      const entity = { id: 1, ...dto } as Author;
  ...
  describe('findAll', () => {
    it('should return paginated result with correct meta and totalPages = ceil(total/limit)', async () => {
      const data = [{ id: 1, name: 'A' }] as Author[];
    ...
    it('should filter by name using Like when search is provided', async () => {
      ...
      expect(repo.findAndCount).toHaveBeenCalledWith(
        expect.objectContaining({ where: { name: Like('%martin%') } }),
      );
    });
  });
  ...
  describe('findOne', () => {
    it('should return the author when found', async () => {
      const entity = { id: 1, name: 'A' } as Author;
  ...
  describe('update', () => {
    it('should merge changes and save when the author exists', async () => {
      const existing = { id: 1, name: 'Old', bio: 'x' } as Author;
      ...
      expect(repo.save).toHaveBeenCalledWith(
        expect.objectContaining({ id: 1, name: 'New', bio: 'x' }),
      );
  ...
  describe('remove', () => {
    it('should remove the author when it exists', async () => {
      const existing = { id: 1, name: 'A' } as Author;
      repo.findOne.mockResolvedValue(existing);
